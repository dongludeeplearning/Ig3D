...
def evaluate_valence(path: str):
    df = pd.read_csv(path)
    discrete = "cat_pred" in df.columns
    va = "val_pred" in df.columns
    if va:
        true_values = list(df["val_true"]) 
        pred_values = list(df["val_pred"]) 
    if va:
        mse = mean_squared_error(true_values, pred_values)
        mae = mean_absolute_error(true_values, pred_values)
        # rmse = root_mean_squared_error(true_values, pred_values)
...
def evaluate_arousal(path: str):
    df = pd.read_csv(path)
    discrete = "cat_pred" in df.columns
    va = "val_pred" in df.columns
    if va:
        true_values = list(df["aro_true"])
        pred_values = list(df["aro_pred"])
    if va:
        mse = mean_squared_error(true_values, pred_values)
        mae = mean_absolute_error(true_values, pred_values)
        # rmse = root_mean_squared_error(true_values, pred_values)
...
for file in os.listdir(path):
    ...
    print ("full valance and arousal")
    evaluate(os.path.join(path, file))

    ## uncomment for getting seperate values
    # print ("only valance")
    # evaluate_valence(os.path.join(path, file))
    # print ("only arousal")
    # evaluate_arousal(os.path.join(path, file))
    print("\n")
    print(50 * "-")
    print("\n")