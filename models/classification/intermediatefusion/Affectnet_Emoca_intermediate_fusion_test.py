...
from networks.DDAM_smirkfusion_feature import EmocaEarlyLinearFUsion 
...
def parse_args():
    ...
    parser.add_argument('--featured_model_path', default = 'final_checkpoint/affecnet_Emoca_156_features_early_linear_fusion_epoch3_acc0.6287.pth')
    parser.add_argument('--aff_path', type=str, default='/home/hbml-user/XiaoWang/FaceData/affectnetAndEmoca', help='AffectNet dataset path.')
...
def run_test():
    ...
    model_base = DDAM()
    model_emoca = EmocaEarlyLinearFUsion()
    ...
    plot_confusion_matrix(matrix, classes=class8_names, normalize=True, title='AffectNet_Emoca_Early_fusion_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"affecnet8_Emoca_Early_Fusion_acc{acc}_{timestamp}.png"))
    plt.close()
