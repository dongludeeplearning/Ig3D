...
def parse_args():
    ...
    parser.add_argument('--featured_model_path', default = 'final_checkpoint/affecnet_358_features_early_linear_fusion_epoch2_acc0.6117.pth')
...
def run_test():
    ...
    model_base = DDAM()
    model_emoca = SMirkEarlyLinearFUsion()
    ...
    plot_confusion_matrix(matrix, classes=class8_names, normalize=True, title='AffectNet_Smirk_Early_fusion_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"affecnet8_Smirk_Early_Fusion_acc{acc}_{timestamp}.png"))
    plt.close()
