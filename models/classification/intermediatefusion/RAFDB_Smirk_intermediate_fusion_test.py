...
from networks.DDAM_smirkfusion_feature import RAFDB_SmirkEarlyLinearFUsion 
...
def parse_args():
    ...
    parser.add_argument('--aff_path', type=str, default='/home/hbml-user/XiaoWang/FaceData/RAF-DBAndSmirk/DATASET', help='RAFDB dataset path.')
    ...
    parser.add_argument('--featured_model_path', default = 'final_checkpoint/RAFDB_SMirk_358_features_early_linear_fusion_epoch26_acc0.9013.pth')
...
def run_test():
    ...
    model_emoca = RAFDB_SmirkEarlyLinearFUsion(num_class=args.num_class, num_head=args.num_head)
    ...
    plot_confusion_matrix(matrix, classes=RAF_DB_CLass, normalize=True, title='RAFDB_Emoca_334_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"RAFDB_Smirk_Early_fusion_acc{acc}.png"))
    plt.close()
