...
def run_test():
    ...
    plot_confusion_matrix(matrix, classes=class8_names, normalize=True, title='AffectNet Eomoca_MaxFusion_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"AffectNet_Emoca_max_fusion_acc{acc}_{timestamp}.png"))
    plt.close()
