...
from networks.SER import AFF_EmocaRecognition as EmocaRecognition
...
def parse_args():
    ...
    parser.add_argument('--featured_model_path', default = 'savedcheckpoints/affecnet_Emoca_156.pth')
...
def run_test():
    ...
    for (imgs_base, targets_base, features_base,val, aro), (imgs_smirk, targets_smirk, features_smirk,val, aro) in zip(val_base_loader, val_smirk_loader):
        ...
        out_fused = 0.8*out_base_primary + 0.2*out_emoca_primary
    ...
    plot_confusion_matrix(matrix, classes=class8_names, normalize=True, title='AffectNet Eomoca__weight_Fusion_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"AffectNet_Emoca_weight_fusion_acc{acc}_{timestamp}.png"))
    plt.close()
