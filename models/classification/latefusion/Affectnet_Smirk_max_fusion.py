...
def run_test():
    ...
    for (imgs_base, targets_base, features_base,val, aro), (imgs_smirk, targets_smirk, features_smirk,val, aro) in zip(val_base_loader, val_smirk_loader):
        ...
        out_fused = torch.max(out_base_primary, out_smirk_primary)
    ...
    plt.savefig(os.path.join('FusionConfusionMatrix', f"AffectNet_Smirk_max_fusion_acc{acc}_{timestamp}.png"))
    plt.close()
