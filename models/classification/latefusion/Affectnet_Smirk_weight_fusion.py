...
from networks.SER import AFF_SmirkEmoRecognition as SmirkEmoRecognition
...
def parse_args():
    ...
    parser.add_argument('--base_model_path', default = 'savedcheckpoints/affecnet8sota.pth')
    parser.add_argument('--featured_model_path', default = 'savedcheckpoints/smirk_358.pth')
...
def run_test():
    ...
    for (imgs_base, targets_base, features_base,val, aro), (imgs_smirk, targets_smirk, features_smirk,val, aro) in zip(val_base_loader, val_smirk_loader):
        ...
        out_smirk_primary = out_smirk[2]

        
        out_fused = 0.77*out_base_primary + 0.23*out_smirk_primary
    ...
    plt.savefig(os.path.join('FusionConfusionMatrix', f"AffectNet_Smirk_weight_fusion_acc{acc}_{timestamp}.png"))
    plt.close()
