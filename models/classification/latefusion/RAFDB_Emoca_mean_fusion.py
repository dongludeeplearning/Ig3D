...
def run_test():
    ...
    for (imgs_base, targets_base, features_base), (imgs_smirk, targets_smirk, features_smirk) in zip(val_base_loader, val_smirk_loader):
        ...
        out_fused = (out_base_primary + out_emoca_primary)/2.0
    ...
    plot_confusion_matrix(matrix, classes=RAF_DB_CLass, normalize=True, title=' Eomoca_mean_Fusion_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"RAFDB_Emoca_mean_fusion_acc{acc}_{timestamp}.png"))
    plt.close()
