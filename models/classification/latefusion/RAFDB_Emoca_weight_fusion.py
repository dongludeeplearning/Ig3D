...
from networks.SER import EmocaRecognition
...
def parse_args():
    ...
    parser.add_argument('--aff_path', type=str, default='/home/hbml-user/XiaoWang/FaceData/RAF-DB/DATASET', help='Emoca dataset path.')
    parser.add_argument('--batch_size', type=int, default=128, help='Batch size.')
    parser.add_argument('--workers', default=8, type=int, help='Number of data loading workers.')
    parser.add_argument('--num_head', type=int, default=2, help='Number of attention head.')
    parser.add_argument('--num_class', type=int, default=7, help='Number of class.')
    parser.add_argument('--num_features', type=int, default=358, help='Number of features.')
    ...
    parser.add_argument('--base_model_path', default = './savedcheckpoints/RAFDB_acc0.9016.pth')
    parser.add_argument('--featured_model_path', default = './savedcheckpoints/RAFDB_Emoca334_epoch31_acc0.7927.pth')

    return parser.parse_args()

class CustomImageFolder(Dataset):
    def __init__(self, root, transform=None, num_classes=8, mask_prob=0.0, mask=True):
        self.root = root
...
def run_test():
    ...
    model_base = DDAM(num_class=args.num_class, num_head=args.num_head)
    ...
    for (imgs_base, targets_base, features_base), (imgs_smirk, targets_smirk, features_smirk) in zip(val_base_loader, val_smirk_loader):
        ...
        out_base_primary = out_base[0]
        out_emoca_primary = out_emoca[0]
        out_fused = 0.65*out_base_primary + 0.35*out_emoca_primary
    ...
    plot_confusion_matrix(matrix, classes=RAF_DB_CLass, normalize=True, title=' Eomoca_Weighted_Fusion_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"RAFDB_Emoca_Weighted_fusion_acc{acc}_{timestamp}.png"))
    plt.close()
