...
def run_test():
    ...
    for (imgs_base, targets_base, features_base), (imgs_smirk, targets_smirk, features_smirk) in zip(val_base_loader, val_smirk_loader):
        ...
        out_base = out_base[0]
        out_fused = torch.max(out_smirk, out_base)
    ...
    plot_confusion_matrix(matrix, classes=RAF_DB_CLass, normalize=True, title='RAFDB_Smirk_max_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"RAFDB_Smirk_max_acc{acc}.png"))
    plt.close()
