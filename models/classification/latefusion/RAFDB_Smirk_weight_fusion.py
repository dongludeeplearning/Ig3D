...
from networks.SER import RAF_SmirkRecognition
...
def parse_args():
    ...
    parser.add_argument('--featured_model_path', default = 'savedcheckpoints/RAFDB_smirk358_epoch37_acc0.7557.pth')
...
def run_test():
    ...
    model_smirk = RAF_SmirkRecognition()
    ...
    model_smirk.load_state_dict(smirk_model_checkpoint['model_state_dict'], strict=False)
    model_base.to(device)
    model_base.eval()
    model_smirk.to(device)
    model_smirk.eval()
    ...
    for (imgs_base, targets_base, features_base), (imgs_smirk, targets_smirk, features_smirk) in zip(val_base_loader, val_smirk_loader):
        ...
        out_smirk = model_smirk(imgs_smirk, features_smirk)
        out_base = out_base[0]
        out_fused = 0.35*out_smirk+0.65*out_base
    ...
    plot_confusion_matrix(matrix, classes=RAF_DB_CLass, normalize=True, title='RAFDB_Smirk_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"RAFDB_Smirk_weight_fusion_acc{acc}.png"))
    plt.close()
