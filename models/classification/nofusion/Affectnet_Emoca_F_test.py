...
from networks.SER import EmocaEmoRecognition_F as EmocaRecognition
...
def parse_args():
    ...
    parser.add_argument('--featured_model_path', default = 'final_checkpoint/Affecnet_Emoca_F.pth')
...
def run_test():
    ...
    plot_confusion_matrix(matrix, classes=class8_names, normalize=True, title='Affecnet_Emoca_Femo_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"Affecnet_Emoca_F_acc{acc}_{timestamp}.png"))
    plt.close()
