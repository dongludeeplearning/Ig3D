...
from networks.SER import EmocaEmoRecognition_S 
...
def parse_args():
    ...
    parser.add_argument('--aff_path', type=str, default='/home/hbml-user/XiaoWang/FaceData/affectnetAndEmoca', help='AffectNet Emoca dataset path.')
...
class CustomImageFolder(Dataset):
    ...
    def __getitem__(self, index): 
        ...
        return image, label, features, aro,val
...
def Emoca_loss(valence_true, arousal_true, expression_true, valence_pred, arousal_pred, expression_pred, alpha, beta, gamma):
    ...
    return 2*ce_loss + (alpha/(alpha+beta+gamma)) * mse +  (beta/(alpha+beta+gamma)) * (1 - pcc) +  (gamma/(alpha+beta+gamma))  * (1 - ccc), ce_loss
...
def run_training():
    ...
    val_loader = get_dataloader(args, f'{args.aff_path}/val', args.val_excel_path, data_transforms_val, args.batch_size, args.workers, mask=False)
    model = EmocaEmoRecognition_S()
    model.to(device)
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        for imgs, targets, features, val, aro in tqdm(train_loader):
            ...
            loss,ce_loss = Emoca_loss(valence_true,arousal_true, expression_true, valence_pred,arousal_pred,expression_pred , alpha,beta,gamma)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            if acc > best_acc:
                ...
                torch.save({'iter': epoch,
                           ...
                           os.path.join('final_checkpoint', "Affecnet_Emoca_S_epoch"+str(epoch)+"_acc"+str(acc)+".pth"))
