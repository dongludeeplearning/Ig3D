...
def parse_args():
    ...
    parser.add_argument('--featured_model_path', default = 'final_checkpoint/Affecnet_Smirk_F.pth')
...
def run_test():
    ...
    plot_confusion_matrix(matrix, classes=class8_names, normalize=True, title='AffectNet_Smirk_F Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"Affecnet__Smirk_F__acc{acc}_{timestamp}.png"))
    plt.close()
