...
from networks.SER import SmirkEmoRecognition_S as SmirkEmoRecognition
...
def parse_args():
    ...
    parser.add_argument('--featured_model_path', default = 'final_checkpoint/Affecnet_Smirk_S.pth')
...
def run_test():
    ...
    model_base = DDAM()
    model_smirk = SmirkEmoRecognition()
    ...
    model_smirk.load_state_dict(smirk_model_checkpoint['model_state_dict'])
    ...
    for (imgs_base, targets_base, features_base,val, aro), (imgs_smirk, targets_smirk, features_smirk,val, aro) in zip(val_base_loader, val_smirk_loader):
        ...
        out_smirk_primary = out_smirk[0]
        out_fused =out_smirk_primary
    ...
    plot_confusion_matrix(matrix, classes=class8_names, normalize=True, title='AffectNet_Smirk_S Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"Affecnet__Smirk_S__acc{acc}_{timestamp}.png"))
    plt.close()
