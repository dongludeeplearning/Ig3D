...
def parse_args():
    ...
    parser.add_argument('--input_dim', type=int, default=334, help='Number of class.')
    ...
    parser.add_argument('--featured_model_path', default = 'final_checkpoint/RAFDB_Emoca_F.pth')
...
def run_test():
    ...
    model_base = DDAM(num_class=7)
    model_emoca= RAFDB_Emoca_Recognition(input_size=args.input_dim)
    ...
    plot_confusion_matrix(matrix, classes=RAF_DB_CLass, normalize=True, title='RAFDB_Emoca_S_Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"RAFDB_Emoca_S_acc{acc}.png"))
    plt.close()
